...
class CertificateScreenRejected(RuntimeError):
    pass


def generate(m,q,delta_g_db=.01,clearance_m=1.,max_bisections=1000):
        ...
        for ei,e in enumerate(task['events']):
            ...
            def cover(lo,hi,depth=0):
                ...
                if mode=='outage' and depth<28 and hi-lo>1e-9:
                    mid=(lo+hi)/2
                    if choose(mid,mid)[0]=='outage':
                        raise CertificateScreenRejected(f'No robust point certificate: {task["id"]} event {ei}, t={mid}')
                    if subdivisions>=max_bisections:
                        raise CertificateScreenRejected('Experimental proof subdivision budget reached')
                    subdivisions+=1;cover(lo,mid,depth+1);cover(mid,hi,depth+1);return
